...
async function handleDropdown(dropdownSelector, optionText, nthIndex) {
  const dropdownType = Selector(dropdownSelector).nth(nthIndex);
  const dropdownOptions = Selector(
    ".dx-item-content, .dx-list-item-content"
  ).withText(optionText);
...
items.new.forEach((item) => {
  test(
    `CreateNewItem: ${item.name} UOM: ${item.UOM} Item Type: ${item.Type} Tracking Mode: ${item.TrackingMode} Costing Method: ${item.CostingMethod}`,
    async (t) => {
      await t.useRole(login);
      await ItemPage.navigateToItemSection();
      await ItemPage.createItem(item.name, item.arabicname);
      await handleDropdown("div.dx-button-content", item.UOM, 9); // Provide UOM
      await handleDropdown("div.dx-button-content", item.Type, 10); // Provide Item Type
      await handleDropdown("div.dx-button-content", item.TrackingMode, 11); // Provide Tracking Mode
      await handleDropdown("div.dx-button-content", item.CostingMethod, 12); // Provide Costing Method
      await t.click("#MainMenu_DXI0_T");
      await assertItemCreated(t, item.name);
    }
  );
});
...
items.delete.forEach((item)=>{
  test(
    `DeleteNewlyCreated Item: ${item.name}`,
    async(t)=>{
      await t.useRole(login);
      await t.wait(2000).click("#leftNavigation_I0i2_");
      await ItemPage.deleteItem(item.name);
      await assertItemDelete(t, "Item deleted successfully");
    }
  );
})